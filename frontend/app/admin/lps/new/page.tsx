...
export default function NewLPPage() {
  ...
  return (
    <div className="p-8 max-w-4xl">
      <h1 className="text-2xl font-bold text-gray-900 mb-6">New Landing Page</h1>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <Section title="Basic Info">
          <div className="grid grid-cols-2 gap-4">
            ...
            <Field label="Slug" value={form.slug} onChange={(v) => set('slug', v)} placeholder="e.g. web-design" />
          </div>
        ...
        <Section title="FAQs">
          {[1, 2, 3, 4, 5].map((n) => (
            <div key={n} className="grid grid-cols-2 gap-4">
              <Field
                label={`Question ${n}`}
                value={form[`faq${n}` as keyof typeof form]}
                onChange={(v) => set(`faq${n}`, v)}
              />
              <Textarea
                label={`Answer ${n}`}
                value={form[`ans${n}` as keyof typeof form]}
